...
float calculateNetworkUsage(const NetworkStats& prev, const NetworkStats& current) {
    ...
    // Convert bytes to Mbps (1 byte = 8 bits, 1 Mbps = 1,000,000 bits)
    return ((sentDelta + recvDelta) * 8.0) / (1024 * 1024);
}
...
int main() {
    ...
    while (true) {
        ...
        float networkUsage = calculateNetworkUsage(prevNetStats, currentNetStats);
        ...
        uint8_t pwmCPU = static_cast<uint8_t>(cpuUsage * 255 / 100);
        uint8_t pwmRAM = static_cast<uint8_t>(ramUsage * 255 / 100);

        // Convert network usage to Mbps and scale it as uint32_t
        uint32_t networkUsageRaw = static_cast<uint32_t>(networkUsage);
